  ...
  return (
    <div className={`p-4 pb-24 space-y-6 ${theme.text}`}>
      <div className="space-y-4">
        <div>
          <label className={`block text-[10px] font-black uppercase tracking-widest mb-2 ${theme.muted}`}>Routine Title</label>
          <input 
            value={name}
        ...
        <div>
          <label className={`block text-[10px] font-black uppercase tracking-widest mb-2 ${theme.muted}`}>Theme Accent</label>
          <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar">
            {['#a78bfa', '#818cf8', '#f472b6', '#34d399', '#fbbf24', '#f87171'].map(c => (
              <button 
                key={c}
                onClick={() => setColor(c)}
                className={`w-10 h-10 rounded-full border-2 flex-shrink-0 transition-all active:scale-90 ${color === c ? 'border-white ring-4 ring-purple-500/30' : 'border-transparent opacity-60'}`}
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between px-1">
          <label className={`block text-[10px] font-black uppercase tracking-widest ${theme.muted}`}>Steps</label>
          <button 
            onClick={addStep}
        ...
        <div className="space-y-4">
          {steps.map((step, idx) => {
            ...
            return (
              <div key={step.id} className={`p-5 rounded-[2rem] ${theme.surface} border ${theme.border} space-y-5 shadow-sm`}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center bg-black/5 rounded-xl p-1">
                    <button onClick={() => moveStep(idx, 'up')} className={`p-1 rounded-lg hover:bg-white/50 ${idx === 0 ? 'opacity-20 pointer-events-none' : ''}`}><ChevronUp size={18} /></button>
                    <button onClick={() => moveStep(idx, 'down')} className={`p-1 rounded-lg hover:bg-white/50 ${idx === steps.length - 1 ? 'opacity-20 pointer-events-none' : ''}`}><ChevronDown size={18} /></button>
                    <div className="w-px h-4 bg-black/10 mx-1"></div>
                    <button onClick={() => removeStep(step.id)} className="p-1 rounded-lg hover:bg-red-50 text-red-400"><Trash2 size={18} /></button>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  <div className="flex-1 flex items-center gap-2">
                    <div className="flex-1">
                      <span className={`text-[9px] font-black ${theme.muted} block mb-1 uppercase tracking-tight`}>Minutes</span>
                      <div className="relative">
                        <input 
                          ...
                          className={`w-full p-3 pr-8 rounded-2xl ${theme.bg} border ${theme.border} outline-none focus:ring-2 focus:ring-purple-500 font-bold text-center tabular-nums`}
                        />
                        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-black opacity-30">m</span>
                      </div>
                    </div>
                    <div className="flex-1">
                      <span className={`text-[9px] font-black ${theme.muted} block mb-1 uppercase tracking-tight`}>Seconds</span>
                      <div className="relative">
                        <input 
                          type="number" 
                          min="0"
                          ...
                          className={`w-full p-3 pr-8 rounded-2xl ${theme.bg} border ${theme.border} outline-none focus:ring-2 focus:ring-purple-500 font-bold text-center tabular-nums`}
                        />
                        <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-black opacity-30">s</span>
                      </div>
                    </div>
                  </div>
                  <div className="w-24">
                    <span className={`text-[9px] font-black ${theme.muted} block mb-1 uppercase tracking-tight`}>Repeats</span>
                    <input 
                      type="number" 
                      min="1"
                      value={step.repeats}
      ...
      <div className="flex gap-4 pt-6 sticky bottom-4 z-20">
        {workout && (
          <button 
            ...
            className="p-5 rounded-3xl border-2 border-red-200 text-red-500 font-black bg-white/90 backdrop-blur-lg hover:bg-red-50 transition-all active:scale-95 flex items-center justify-center"
          >
            <Trash2 size={24} />
          </button>
        )}
